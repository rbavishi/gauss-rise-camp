"""
Houses the random data-frame generation logic.
"""
...
class Bags:
    ...
    five_ints = ValueBag(list(range(15, 55, 5)), "how-much")
    more_ints = ValueBag([3, 123, 532, 391, 53, 483, 85, 584, 48, 68, 49], "more-ints")
    ...
    even_floats = ValueBag([x / 2 for x in range(-10, 10, 1)], "div-by-twos")
    big_floats = ValueBag([132141.124, 132186.432, 3024234.234, 4234.4, 894324.5, 23894243.7, 123.4, np.NaN], "wats")
    no_nans_floats = ValueBag([71.3, 123.4, 32.4, 85.5, 23.7, 23.8, 83.7], "no-nans")
    more_nans_floats = ValueBag([123.4, 2324.2, 213.789, np.NaN, np.NaN, np.NaN, np.NaN, np.NaN], "more-nans")
...
class DfConfig(NamedTuple):
    ...
    index_like_columns_prob: float = 0.15
    col_prefix: str = ''  # A common prefix for all columns
    nan_prob: float = 0.05  # Probability of having nans (apart from the values in the float/nan bags being picked)
...
def generate_random_dataframe(cfg: DfConfig = None):
    ...
    if index_levels is None and CoinToss(bias=cfg.multi_index_prob) == 1 and cfg.max_index_levels >= 2:
        index_levels = SelectRange(low=2, high=cfg.max_index_levels)

    column_levels = cfg.column_levels
    if column_levels is None and CoinToss(bias=cfg.multi_col_index_prob) == 1 and cfg.max_column_levels >= 2:
...
#  For quick testing on IDEs like PyCharm
if __name__ == "__main__":
    print(generate_random_dataframe(DfConfig()))
